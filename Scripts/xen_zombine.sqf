...
_unitWithSword setVariable ["WBK_SynthHP",350,true];
...
[
["WBK_HL_Zombine_zombine_idle1","WBK_HL_Zombine_zombine_idle2","WBK_HL_Zombine_zombine_idle3","WBK_HL_Zombine_zombine_idle4"],
["WBK_HL_Zombine_zombine_alert6","WBK_HL_Zombine_zombine_alert5","WBK_HL_Zombine_zombine_alert4","WBK_HL_Zombine_zombine_alert3","WBK_HL_Zombine_zombine_alert2","WBK_HL_Zombine_zombine_alert1"],
["WBK_HL_Zombine_zombine_alert6","WBK_HL_Zombine_zombine_alert5","WBK_HL_Zombine_zombine_alert4","WBK_HL_Zombine_zombine_alert3","WBK_HL_Zombine_zombine_alert2","WBK_HL_Zombine_zombine_alert1"],
["WBK_HL_Zombine_zombine_die1","WBK_HL_Zombine_zombine_die2","WBK_HL_Zombine_zombine_die3","WBK_HL_Zombine_zombine_die4"],
["WBK_HL_Zombine_zombine_die1","WBK_HL_Zombine_zombine_die2","WBK_HL_Zombine_zombine_die3","WBK_HL_Zombine_zombine_die4"]
]];
...
[ 
    "OEC_Magazine_AR2", 
    "OEC_Magazine_PulseSMG", 
    "OEC_Magazine_OICW",
	"OEC_SMG46_Mag"
];
_selectedMag = selectRandom _magazinePool;

if (_selectedMag == "OEC_Magazine_AR2" || _selectedMag == "OEC_Magazine_OICW") then 
{
	for [{private _i = 0}, {_i < 3}, {_i = _i + 1}] do 
	{
		_ammoCount = random 30;
		_unitWithSword addMagazine [_selectedMag, _ammoCount];
	};
};
if (_selectedMag == "OEC_Magazine_PulseSMG") then 
{
	for [{private _i = 0}, {_i < 3}, {_i = _i + 1}] do 
	{
		_ammoCount = random 60;
		_unitWithSword addMagazine [_selectedMag, _ammoCount];
	};
};
if (_selectedMag == "OEC_SMG46_Mag") then 
{
	for [{private _i = 0}, {_i < 3}, {_i = _i + 1}] do 
	{
		_ammoCount = random 45;
		_unitWithSword addMagazine [_selectedMag, _ammoCount];
	};
};
_unitWithSword addMagazines ["OEC_Magazine_MK3A2", 1];
	...
	if (((_en distance _mutant) <= 2) and (count _ins == 0) and !(gestureState _mutant in ["wbk_zombine_grenade_loop","wbk_zombine_grenade_in","wbk_zombie_attack_left","wbk_zombie_attack_right"])) then {
		switch true do {
			case (_mutant getVariable "WBK_AI_ZombieMoveSet" == "WBK_Crawler_Idle"): {
				[_mutant, "WBK_Crawler_Attack"] remoteExec ["switchMove", 0];
				[_mutant, "WBK_Crawler_Idle"] remoteExec ["playMoveNow", 0];
			...
			case ((random 100) >= 80): {
				_mutant spawn {
					...
					uisleep 0.8;
					if (!(alive _this) or !(gestureState _this == "WBK_Zombine_Grenade_In")) exitWith {};
					[_this, selectRandom ["WBK_HL_Zombine_zombine_readygrenade1","WBK_HL_Zombine_zombine_readygrenade2"], 50, 6] execVM "\WebKnight_StarWars_Mechanic\createSoundGlobal.sqf";
